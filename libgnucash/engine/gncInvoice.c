...
GncInvoice *gncInvoiceCopy (const GncInvoice *from)
{
    ...
    qof_instance_get_kvp (QOF_INSTANCE (from), &v, GNC_INVOICE_IS_CN);
    if (G_VALUE_HOLDS_INT64 (&v))
         qof_instance_set_kvp (QOF_INSTANCE (invoice), &v, GNC_INVOICE_IS_CN);
...
gboolean gncInvoiceGetIsCreditNote (const GncInvoice *invoice)
{
    GValue v = G_VALUE_INIT;
    if (!invoice) return FALSE;
    qof_instance_get_kvp (QOF_INSTANCE(invoice), &v, GNC_INVOICE_IS_CN);
