...
gnc_commodity_get_auto_quote_control_flag(const gnc_commodity *cm)
{
    ...
    if (!cm) return FALSE;
    qof_instance_get_kvp (QOF_INSTANCE (cm), &v, "auto_quote_control");
...
gnc_commodity_get_user_symbol(const gnc_commodity *cm)
{
    GValue v = G_VALUE_INIT;
    if (!cm) return NULL;
    qof_instance_get_kvp (QOF_INSTANCE(cm), &v, "user_symbol");
    if (G_VALUE_HOLDS_STRING (&v))
        ...
        const gboolean flag)
{
    ...
    gnc_commodity_begin_edit(cm);
    if (flag)
        qof_instance_set_kvp (QOF_INSTANCE (cm), NULL, "auto_quote_control");
    else
    {
        g_value_init (&v, G_TYPE_STRING);
        g_value_set_string (&v, "false");
        qof_instance_set_kvp (QOF_INSTANCE (cm), &v, "auto_quote_control");
    }
    mark_commodity_dirty(cm);
    gnc_commodity_commit_edit(cm);
    LEAVE("");
...
gnc_commodity_set_user_symbol(gnc_commodity * cm, const char * user_symbol)
{
    ...
    if (user_symbol)
    {
        ...
        qof_instance_set_kvp (QOF_INSTANCE(cm), &v, "user_symbol");
    }
    else
        qof_instance_set_kvp (QOF_INSTANCE(cm), NULL, "user_symbol");
